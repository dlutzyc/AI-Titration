# 基于计算机视觉的酸碱滴定控制装置
Automatic Titration Control System

**[慕乐网络科技(大连)有限公司, MoolsNet](https://www.mools.net/)**

![Logo](assets/组合logo.png?raw=true)

简单写一下，用来介绍软件包的层级。也为了显得专业一点。

这个软件包提供的是示例程序，理论上大家直接执行也可以完成整个滴定控制过程，到那时我们还是希望大家可以发挥自己的创造力和奇思妙想，选用不用的控制方式、神经网络等，也许可以获得更高的创意分哟

**基于计算机视觉的酸碱滴定控制装置(Automatic_Titration_Control)** 软件包中的示例程序分别放置在3个文件夹中，此外附带了3个说明文件和1份附录文件夹，之所以这样做是比较符合一般常见的Python软件包的模式。
...
**Picture_Train** 该文件夹存放 **使用pytorch实现的代码版本**。这里列出了十余种神经网络的训练和测试程序，欢迎大家尝试使用。详情参见此文件夹中的`README.md`文件。
...
**Input** 文件夹里面放了我们之前拍的许多照片，大家可以拿来做训练集 
